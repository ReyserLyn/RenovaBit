...
// One row per tier; the customer column is the only non-admin margin.
const rules = [
	{ minPrice: "0", maxPrice: "100", customerPct: "25" },
	{ minPrice: "100", maxPrice: "200", customerPct: "20" },
	{ minPrice: "200", maxPrice: "800", customerPct: "15" },
...
describe("lookupMarginRule", () => {
	it("finds rule for supplier price 0", () => {
		...
		expect(result).toEqual({ customerPct: "25" });
	});
	...
	it("moves to next tier when price equals maxPrice (exclusive)", () => {
		...
		expect(result).toEqual({ customerPct: "20" });
	});
	...
	it("finds rule for price 200", () => {
		...
		expect(result).toEqual({ customerPct: "15" });
	});
	...
	it("finds rule for price 800", () => {
		...
		expect(result).toEqual({ customerPct: "10" });
	});
