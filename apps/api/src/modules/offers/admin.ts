...
export const adminOffersRoute = new Elysia({ prefix: "/offers" })
	...
	.post(
		"/:id/brands",
		async ({ params: { id }, body }) => OfferService.assignBrands(id, body.brandIds),
		{
			isAdmin: true,
			params: OfferModel.idParams,
			body: OfferModel.brandAssignBody,
			response: {
				200: OfferModel.assignResponse,
				400: OfferModel.errorResponse,
				401: OfferModel.errorResponse,
			...
			detail: { summary: "Asignar marcas a oferta", tags: ["Offers"] },
		},
	)

	// ── Get brands for offer ──────────────────────
	.get("/:id/brands", async ({ params: { id } }) => OfferService.getBrandsWithDetails(id), {
		isAdmin: true,
		params: OfferModel.idParams,
		response: {
			200: OfferModel.offerBrandDetailResponse,
			401: OfferModel.errorResponse,
			403: OfferModel.errorResponse,
			404: OfferModel.errorResponse,
		},
		detail: { summary: "Marcas asignadas a oferta", tags: ["Offers"] },
	})

	// ── Assign categories ─────────────────────────
	.post(
		"/:id/categories",
		async ({ params: { id }, body }) => OfferService.assignCategories(id, body.categoryIds),
		{
			isAdmin: true,
			params: OfferModel.idParams,
			body: OfferModel.categoryAssignBody,
			response: {
				200: OfferModel.assignResponse,
				400: OfferModel.errorResponse,
				401: OfferModel.errorResponse,
			...
			detail: { summary: "Asignar categorías a oferta", tags: ["Offers"] },
		},
	)

	// ── Get categories for offer ──────────────────
	.get("/:id/categories", async ({ params: { id } }) => OfferService.getCategoriesWithDetails(id), {
		isAdmin: true,
		params: OfferModel.idParams,
		response: {
			200: OfferModel.offerCategoryDetailResponse,
			401: OfferModel.errorResponse,
			403: OfferModel.errorResponse,
			404: OfferModel.errorResponse,
		},
		detail: { summary: "Categorías asignadas a oferta", tags: ["Offers"] },
	});
