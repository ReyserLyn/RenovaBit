/**
 * Prompt v1, archived for comparison only.
 *
 * This is the prompt that ran in production until v2 replaced it. Keep it so the
 * eval can keep measuring "what we had" against "what we have", and so the
 * regression is documented: v1 asked for names of "moderate length", which is
 * what made the model drop specs like "5FAN ARGB" or "1ms".
 */

export type ExtractionContextV1 = {
	categories: string[];
	brands: string[];
};
...
export function buildExtractionPromptV1(rawName: string, context: ExtractionContextV1): string {
	const catList =
