...
export const useFileUpload = (
	...
	// Track files in a ref for unmount cleanup (avoids stale closure in effect)
	...
	const validateFile = useCallback(
		(file: File | FileMetadata): string | null => {
			if (file.size > maxSize) {
				return `"${file.name}" excede el tamaño máximo de ${formatBytes(maxSize)}.`;
			}
			...
			if (accept !== "*") {
				...
				if (!isAccepted) {
					return `"${file instanceof File ? file.name : file.name}" no es un tipo de archivo aceptado.`;
				}
			}
