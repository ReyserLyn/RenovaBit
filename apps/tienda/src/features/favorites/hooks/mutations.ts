...
function prependToInfiniteCaches(
	...
	queryClient.setQueriesData<{ pages: FavoriteListResponse[]; pageParams: number[] }>(
		{
...
export function useAddFavorite() {
	...
	return useMutation({
		...
		onMutate: ({ productId, snapshot }) => {
			...
			queryClient.setQueryData(favoritesKeys.detail(productId), { isFavorite: true });
		...
		onError: (error, { productId }) => {
			queryClient.invalidateQueries({ queryKey: favoritesKeys.detail(productId) });
			invalidateFavoritesQueries(queryClient);
...
export function useRemoveFavorite() {
	...
	return useMutation({
		...
		onMutate: (productId) => {
			queryClient.setQueryData(favoritesKeys.detail(productId), { isFavorite: false });
		},
		onSuccess: () => {
			invalidateFavoritesQueries(queryClient);
		},
