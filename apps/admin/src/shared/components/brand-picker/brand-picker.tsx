		...
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-2xl p-0 gap-0 max-h-[85dvh] flex flex-col overflow-hidden">
				<DialogHeader className="shrink-0 p-4">
					<DialogTitle>Seleccionar marcas</DialogTitle>
					<DialogDescription>
						{Object.keys(rowSelection).filter((id) => rowSelection[id]).length > 0
							? `${Object.keys(rowSelection).filter((id) => rowSelection[id]).length} marca(s) seleccionada(s)`
							: "Selecciona las marcas que deseas incluir."}
					</DialogDescription>
				</DialogHeader>
