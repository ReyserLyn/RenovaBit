	...
	const menuItems = useMemo(() => {
		...
		if (canSort) {
			...
			items.push(
				...
				>
					...
					{isSorted === "asc" && (
						<HugeiconsIcon icon={Tick02Icon} className="text-primary size-4 opacity-100!" />
					)}
				</DropdownMenuItem>,
				<DropdownMenuItem
					key="sort-desc"
		...
		if (props.tableLayout?.columnsPinnable && canPin) {
			...
			items.push(
				...
				>
					<HugeiconsIcon icon={ArrowLeft02Icon} className="size-3.5!" aria-hidden="true" />
				...
				>
					<HugeiconsIcon icon={ArrowRight02Icon} className="size-3.5!" aria-hidden="true" />
