...
function getPickerColumns(): ColumnDef<{
	...
	return [
		...
		},
		{
			...
			header: "Precio",
			cell: ({ row }) => (
				<span className="font-mono tabular-nums text-sm">S/ {row.original.price}</span>
			),
		},
		{
			accessorKey: "stock",
			header: "Stock",
			cell: ({ row }) => (
				<span className="font-mono tabular-nums text-sm">{row.original.stock}</span>
			),
		},
		{
...
export function ProductPicker({
	...
	return (
		<Dialog open={open} onOpenChange={handleOpenChange}>
			<DialogContent className="sm:max-w-3xl p-0 gap-0 max-h-[85dvh] flex flex-col overflow-hidden">
				<DialogHeader className="shrink-0 p-4">
					<DialogTitle>Seleccionar productos</DialogTitle>
					<DialogDescription>
						{Object.keys(rowSelection).filter((id) => rowSelection[id]).length > 0
							? `${Object.keys(rowSelection).filter((id) => rowSelection[id]).length} producto(s) seleccionado(s)`
