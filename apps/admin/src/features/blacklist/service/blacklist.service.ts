...
type AddBody = Parameters<typeof api.api.v1.admin.scraping.blacklist.post>[0];
type RemoveBody = Parameters<typeof api.api.v1.admin.scraping.blacklist.delete>[0];

// ── API Functions ────────────────────────────────────

async function list(source?: string): Promise<BlacklistEntry[]> {
	const query = source ? { source } : undefined;
	return unwrapResponse(api.api.v1.admin.scraping.blacklist.get({ query }));
}

async function add(data: AddBody): Promise<{ entry: BlacklistEntry; productDeleted: boolean }> {
	return unwrapResponse(api.api.v1.admin.scraping.blacklist.post(data));
}

async function remove(data: RemoveBody): Promise<{ deleted: boolean }> {
	return unwrapResponse(api.api.v1.admin.scraping.blacklist.delete(data));
}
